...
def test_a_fanned_test_runs_inside_its_source(cwd_probe, workers):
...
@pytest.mark.parametrize("workers", WORKERS)
def test_an_unfanned_test_keeps_the_invocation_directory(cwd_probe, workers):
    pytester, recorded = cwd_probe
...
@pytest.mark.parametrize("workers", WORKERS)
def test_no_chdir_keeps_the_invocation_directory(cwd_probe, workers):
    pytester, recorded = cwd_probe
...
@pytest.mark.parametrize("workers", WORKERS)
def test_a_source_reading_at_import_finds_its_own_file(pytester, workers):
...
def test_the_run_leaves_the_calling_directory_alone(cwd_probe):
...
def test_a_failing_test_does_not_leave_the_directory_moved(pytester):
...
def test_no_chdir_still_allows_moving_in_by_hand(pytester):
...
def test_no_chdir_applies_to_a_whole_module(cwd_probe):
    pytester, recorded = cwd_probe
    pytester.makepyfile(
        """
        import os, pathlib, pytest
